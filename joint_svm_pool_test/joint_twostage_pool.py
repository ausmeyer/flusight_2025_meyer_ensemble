...
- Stage 1: SVM (LinearSVR) on transformed target (mu)
...
def distribution_for_mode(sigma_mode: str):
    if sigma_mode == "wide":
        return GaussianFrozenLocBoundedWide(sigma_min=0.1, sigma_max=1.0)
...
def fit_two_stage_one_bag(
    ...
    seed: int,
    svm_max_iter: int,
) -> Tuple[StandardScaler, LinearSVR, LightGBMLSS]:
    ...
    stage1.fit(X_scaled, y)

    mu_train = stage1.predict(X_scaled)
    ...
    d2 = lgb.Dataset(X_scaled, label=y, init_score=init_score, params={"verbose": -1}, free_raw_data=False)
...
def predict_quantiles(
    ...
    X = test_X.astype(float).to_numpy()
    X_scaled = scaler.transform(X)
    mu_model = stage1.predict(X_scaled)
    dist_params = stage2.predict(X_scaled, pred_type="parameters")
...
def forecast_for_anchor(
    ...
    for b in range(cfg.num_bags):
        ...
        if len(bag_train) < cfg.min_train_rows:
            continue
        try:
            scaler, stage1, stage2 = fit_two_stage_one_bag(
...
def build_common_config(args: argparse.Namespace) -> RuntimeConfig:
    ...
    # if --svm-max-iter is not provided, derive from --stage1-rounds
    # so users can still "turn down iterations" via stage1_rounds.
    svm_max_iter = args.svm_max_iter if args.svm_max_iter is not None else max(500, int(args.stage1_rounds) * 100)
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Joint SVM + LightGBMLSS pooled-horizon bagged forecasting test module")
    ...
    def add_common(p: argparse.ArgumentParser) -> None:
        ...
        p.add_argument(
            ...
            type=int,
            default=None,
            help="LinearSVR max_iter for stage 1 (default derives from --stage1-rounds * 100)",
        )
        p.add_argument(
            "--own-lags",
            type=str,
