...
OUTPUT_DIR <- "joint_svm_pool_test/outputs/ensemble_hedge"
...
if (is.na(JOINT_PROSP_FILE)) {
  JOINT_PROSP_FILE <- latest_file("joint_svm_pool_test/outputs", "^prospective.*\\.csv$")
}
...
if (INCLUDE_JOINT_TWOSTAGE) {
  ...
  if (is.na(JOINT_PROSP_FILE) || !file.exists(JOINT_PROSP_FILE)) {
    message("Joint prospective file not found; JointSVMPool will affect weight estimation but not current blended forecast values.")
  }
}
...
for (h in 1:4) {
  ...
  if (length(retro_models) > 0) {
    if ('JointSVMPool' %in% names(retro_models)) {
      n_joint_refs <- retro_models[['JointSVMPool']] %>%
      ...
      if (n_joint_refs < HISTORY_WEEKS) {
        message(sprintf(
          "H%d: JointSVMPool has %d historical refs before as_of (target history=%d); weights may be noisy.",
