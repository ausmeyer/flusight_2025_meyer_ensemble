...
OUTPUT_DIR <- "joint_svm_pool_test/outputs/ensemble_hedge_retrospective"
INCLUDE_ARIMA <- TRUE
INCLUDE_SVM <- TRUE
...
if (is.na(JOINT_RETRO_FILE)) {
  JOINT_RETRO_FILE <- latest_file("joint_svm_pool_test/outputs", "^backtest_cov.*\\.csv$")
}

if (INCLUDE_JOINT_TWOSTAGE) {
  message(sprintf("Backtest hedge: JointSVMPool enabled. retro=%s | ref_shift_days=%d",
  ...
  if (is.na(JOINT_RETRO_FILE) || !file.exists(JOINT_RETRO_FILE)) {
    message("Joint retrospective file not found; running hedge backtest without JointSVMPool.")
  }
}
...
for (h in 1:4) {
  model_dfs <- list()
  if (INCLUDE_ARIMA) {
    model_dfs <- append(model_dfs, list(load_prospective_files(sprintf("^ARIMA_h%d_prospective_\\d{8}\\.csv$", h), "ARIMA")))
  }
  if (INCLUDE_SVM) {
    model_dfs <- append(model_dfs, list(load_prospective_files(sprintf("^SVM_h%d_prospective_\\d{8}\\.csv$", h), "SVM")))
  }
  if (INCLUDE_LGBM_BLENDED) {
    model_dfs <- append(model_dfs, list(load_prospective_files(sprintf("^LGBM-blended_h%d_prospective_\\d{8}\\.csv$", h), "LGBM_blended")))
  }
