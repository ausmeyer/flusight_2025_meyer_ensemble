...
OUTPUT_DIR <- "joint_knn3_pool_test/outputs/ensemble_hedge_retrospective"
INCLUDE_ARIMA <- TRUE
INCLUDE_SVM <- TRUE
...
if (INCLUDE_JOINT_TWOSTAGE) {
  ...
  if (is.na(JOINT_RETRO_FILE) || !file.exists(JOINT_RETRO_FILE)) {
    message("Joint retrospective file not found; running hedge backtest without JointKNN3Pool.")
  }
}
