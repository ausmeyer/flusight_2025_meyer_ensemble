...
- Stage 1: KNN(k=3) on transformed target (mu)
...
def fit_two_stage_one_bag(
    ...
    sigma_mode: str,
    seed: int,
) -> Tuple[StandardScaler, KNeighborsRegressor, LightGBMLSS]:
    X = X_train.astype(float).to_numpy()
    y = np.asarray(y_train_model, dtype=float)

    # Scale features for KNN distance computations.
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    stage1 = KNeighborsRegressor(n_neighbors=3, weights="distance", metric="minkowski")
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Joint KNN3 + LightGBMLSS pooled-horizon bagged forecasting test module")
