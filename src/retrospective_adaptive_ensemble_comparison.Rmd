...
subtitle: "Per-horizon and combined WIS and relative WIS since 2024-10-01"
...
cat("\nDEBUG INFO:\n")
cat("EVAL_START:", as.character(EVAL_START), "\n")
cat("Available dates per model (min/max):\n")
print(wis_scores_all_df %>% group_by(model) %>% summarise(start=min(target_end_date), end=max(target_end_date), n=n()))
cat("Common evaluation set date range:\n")
print(wis_scores_common %>% summarise(start=min(target_end_date), end=max(target_end_date), n_dates=n_distinct(target_end_date)))
cat("\n")
...
cat("\nPER-HORIZON GEOMETRIC MEAN WIS AND RELATIVE WIS (>= 2025-11-01):\n\n")
...
cat("\nCOMBINED GEOMETRIC MEAN WIS AND RELATIVE WIS (All horizons, >= 2024-10-01):\n\n")
