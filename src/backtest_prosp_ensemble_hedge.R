...
OUTPUT_DIR <- "forecasts/retrospective/ensemble_hedge"
INCLUDE_ARIMA <- TRUE
INCLUDE_SVM <- TRUE
...
for (h in 1:4) {
  ...
  ref_dates <- ref_dates[ref_dates >= season_start & ref_dates <= season_end]
  if (length(ref_dates) == 0) {
    message(sprintf("H%d: No reference dates after season filter; skipping.", h))
    next
  }
