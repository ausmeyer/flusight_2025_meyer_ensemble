# Joint Two-Stage Pooled-Horizon Bagged Model (Test Module)
...
- Two-stage training:
  - Stage 1: LightGBM for log-mean (`mu_log`)
...
python joint_twostage_pool_test/joint_twostage_pool.py backtest \
  ...
  --output joint_twostage_pool_test/outputs/backtest_quick.csv \
  --num-bags 20 \
  --bag-frac 0.7 \
  --anchor-step-weeks 2 \
  --max-horizons 4
...
python joint_twostage_pool_test/joint_twostage_pool.py backtest \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --start-date 2025-10-04 \
  --output joint_twostage_pool_test/outputs/backtest_full.csv \
  --num-bags 100 \
  --bag-frac 0.7 \
  --anchor-step-weeks 1 \
  --max-horizons 4 \
...
python joint_twostage_pool_test/joint_twostage_pool.py backtest \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --start-date 2025-10-04 \
  --output joint_twostage_pool_test/outputs/backtest_to_end.csv \
  --num-bags 100 \
  --bag-frac 0.7 \
  --anchor-step-weeks 1 \
  --max-horizons 4 \
...
python joint_twostage_pool_test/joint_twostage_pool.py prospective \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --output joint_twostage_pool_test/outputs/prospective_quick.csv \
  --num-bags 20 \
  --bag-frac 0.7 \
  --max-horizons 4
```
...
python joint_twostage_pool_test/joint_twostage_pool.py prospective \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --output joint_twostage_pool_test/outputs/prospective_full.csv \
  --num-bags 100 \
  --bag-frac 0.7 \
  --max-horizons 4 \
  --sigma-mode wide
...
- Donor-state covariates are enabled by default:
  ...
  - `--cov-min-overlap 40`
- Target mode:
  - `--target-mode level` (default): fit log-level target
  - `--target-mode delta_log`: fit log-change target `log(1+y_{t+h}) - log(1+y_t)` (Flusion-like)
...
python joint_twostage_pool_test/joint_twostage_pool.py backtest \
  ...
  --output joint_twostage_pool_test/outputs/backtest_cov3.csv \
  --max-horizons 4 \
  --cov-top-k 3 \
  --cov-lags 1,2,3,4,8,12,52 \
...
- `joint_twostage_pool_test/ensemble/generate_prosp_adaptive_ensemble_hedge_test.R`

It keeps the existing hedge weighting logic, and adds `JointTwoStagePool` as an optional component using:
...
python joint_twostage_pool_test/joint_twostage_pool.py backtest \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --start-date 2025-11-19 \
  --output joint_twostage_pool_test/outputs/backtest_cov5_delta_log_from_2025-11-19.csv \
  --max-horizons 4 \
  --num-bags 100 \
  --bag-frac 0.7 \
  --cov-top-k 5 \
...
python joint_twostage_pool_test/joint_twostage_pool.py prospective \
  --data-file data/imputed_sets/imputed_and_stitched_hosp_2026-02-07.csv \
  --output joint_twostage_pool_test/outputs/prospective_cov5_delta_log.csv \
  --max-horizons 4 \
  --num-bags 100 \
  --bag-frac 0.7 \
  --cov-top-k 5 \
...
```bash
Rscript joint_twostage_pool_test/ensemble/generate_prosp_adaptive_ensemble_hedge_test.R \
  --asof-date 2026-02-07 \
  --include-joint-twostage true \
  --joint-retro-file joint_twostage_pool_test/outputs/backtest_cov5_delta_log_from_2025-11-19.csv \
  --joint-prosp-file joint_twostage_pool_test/outputs/prospective_cov5_delta_log.csv
```

Output is written to:

- `joint_twostage_pool_test/outputs/ensemble_hedge/AdaptiveEnsemble-hedge_prospective_<YYYYMMDD>.csv`
...
To backtest the hedge ensemble itself with `JointTwoStagePool` included as a component, use:

- `joint_twostage_pool_test/ensemble/backtest_prosp_ensemble_hedge_test.R`

Example:

```bash
Rscript joint_twostage_pool_test/ensemble/backtest_prosp_ensemble_hedge_test.R \
...
- Output goes to `joint_twostage_pool_test/outputs/ensemble_hedge_retrospective/`.
