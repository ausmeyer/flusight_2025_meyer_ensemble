...
while (i <= length(args)) {
  ...
  if (key == '--knn3-prosp-file') { KNN3_PROSP_FILE <- val; i <- i + 2; next }
...
if (is.na(JOINT_PROSP_FILE)) {
  JOINT_PROSP_FILE <- latest_file("joint_twostage_pool_test/outputs", "^prospective.*\\.csv$")
}
if (is.na(KNN3_RETRO_FILE)) {
  KNN3_RETRO_FILE <- latest_file("joint_knn3_pool_test/outputs", "^backtest_cov.*\\.csv$")
}
if (is.na(KNN3_PROSP_FILE)) {
  KNN3_PROSP_FILE <- latest_file("joint_knn3_pool_test/outputs", "^prospective.*\\.csv$")
}
...
if (INCLUDE_JOINT_TWOSTAGE) {
  ...
  if (is.na(JOINT_PROSP_FILE) || !file.exists(JOINT_PROSP_FILE)) {
    message("Joint prospective file not found; JointTwoStagePool will affect weight estimation but not current blended forecast values.")
  }
}
if (INCLUDE_JOINT_KNN3) {
  message(sprintf("Joint KNN3 model enabled. retro=%s | prosp=%s | ref_shift_days=%d",
                  ifelse(is.na(KNN3_RETRO_FILE), "NA", KNN3_RETRO_FILE),
                  ifelse(is.na(KNN3_PROSP_FILE), "NA", KNN3_PROSP_FILE),
                  KNN3_REFERENCE_SHIFT_DAYS))
  if (is.na(KNN3_PROSP_FILE) || !file.exists(KNN3_PROSP_FILE)) {
    message("KNN3 prospective file not found; JointKNN3Pool will affect weight estimation but not current blended forecast values.")
  }
}
if (INCLUDE_ARGO) {
  message(sprintf("ARGO enabled. base=%s", ARGO_BASE))
  if (!dir.exists(ARGO_BASE)) {
    message("ARGO base directory not found; running without ARGO components.")
  }
}
...
load_argo_retro_component <- function(model_name, source_model, h) {
  if (!INCLUDE_ARGO || !dir.exists(ARGO_BASE)) return(NULL)
...
load_argo_prosp_component <- function(model_name, source_model, h, submission_ref_date) {
  if (!INCLUDE_ARGO || !dir.exists(ARGO_BASE)) return(NULL)

  fp <- file.path(ARGO_BASE, sprintf("%s_%s.csv", model_name, format(submission_ref_date, "%Y-%m-%d")))
...
load_retro_for_h <- function(h) {
  ...
  if (INCLUDE_JOINT_KNN3 && !is.na(KNN3_RETRO_FILE) && file.exists(KNN3_RETRO_FILE)) {
    knn3_df <- read_csv(KNN3_RETRO_FILE, show_col_types = FALSE)
    if ('output_type' %in% names(knn3_df) && 'horizon' %in% names(knn3_df)) {
      knn3_df <- knn3_df %>%
        mutate(
          horizon = as.integer(horizon),
        ...
        filter(output_type == 'quantile', horizon == (h - 1))
      if (nrow(knn3_df) > 0) {
        lst$JointKNN3Pool <- knn3_df
      }
    }
  }
  argo_retro <- list(
    load_argo_retro_component("argo_smooth", "ARGO_smooth", h),
    load_argo_retro_component("argo_smooth_log", "ARGO_smooth_log", h),
    load_argo_retro_component("argo2_smooth", "ARGO2_smooth", h),
    load_argo_retro_component("argo2_smooth_log", "ARGO2_smooth_log", h)
  )
  argo_retro <- Filter(function(x) !is.null(x), argo_retro)
  if (length(argo_retro) > 0) {
    for (df in argo_retro) {
      src <- unique(df$source_model)
      if (length(src) == 1) lst[[src]] <- df
    }
  }
  lst
}
...
load_prosp_for_h <- function(h, ts) {
  ...
  if (INCLUDE_JOINT_KNN3 && !is.na(KNN3_PROSP_FILE) && file.exists(KNN3_PROSP_FILE)) {
    knn3_df <- read_csv(KNN3_PROSP_FILE, show_col_types = FALSE)
    if ('output_type' %in% names(knn3_df) && 'horizon' %in% names(knn3_df)) {
      knn3_df <- knn3_df %>%
        mutate(
          horizon = as.integer(horizon),
        ...
        filter(output_type == 'quantile', horizon == (h - 1))
      if ('reference_date' %in% names(knn3_df) && nrow(knn3_df) > 0) {
        if (any(knn3_df$reference_date == submission_ref_date, na.rm = TRUE)) {
          knn3_df <- knn3_df %>% filter(reference_date == submission_ref_date)
        } else {
          latest_ref <- max(knn3_df$reference_date, na.rm = TRUE)
          knn3_df <- knn3_df %>% filter(reference_date == latest_ref)
        }
      }
      if (nrow(knn3_df) > 0) {
        lst$JointKNN3Pool <- knn3_df
  ...
  argo_prosp <- list(
    load_argo_prosp_component("argo_smooth", "ARGO_smooth", h, submission_ref_date),
    load_argo_prosp_component("argo_smooth_log", "ARGO_smooth_log", h, submission_ref_date),
    load_argo_prosp_component("argo2_smooth", "ARGO2_smooth", h, submission_ref_date),
    load_argo_prosp_component("argo2_smooth_log", "ARGO2_smooth_log", h, submission_ref_date)
  )
  argo_prosp <- Filter(function(x) !is.null(x), argo_prosp)
  if (length(argo_prosp) > 0) {
    for (df in argo_prosp) {
      src <- unique(df$source_model)
      if (length(src) == 1) lst[[src]] <- df
    }
  }
...
for (h in 1:4) {
  ...
  if (length(retro_models) > 0) {
    if ('JointTwoStagePool' %in% names(retro_models)) {
      n_joint_refs <- retro_models[['JointTwoStagePool']] %>%
      ...
      if (n_joint_refs < HISTORY_WEEKS) {
        message(sprintf(
          "H%d: JointTwoStagePool has %d historical refs before as_of (target history=%d); weights may be noisy.",
          h, n_joint_refs, HISTORY_WEEKS
        ))
      }
    }
    if ('JointKNN3Pool' %in% names(retro_models)) {
      n_knn_refs <- retro_models[['JointKNN3Pool']] %>%
        mutate(reference_date = as.Date(reference_date)) %>%
        filter(reference_date < as_of_date) %>%
        distinct(reference_date) %>%
        nrow()
      if (n_knn_refs < HISTORY_WEEKS) {
        message(sprintf(
          "H%d: JointKNN3Pool has %d historical refs before as_of (target history=%d); weights may be noisy.",
          h, n_knn_refs, HISTORY_WEEKS
        ))
      }
    }
